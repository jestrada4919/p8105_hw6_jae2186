...
Given data provided for 4000 children, the following code runs a
regression analysis with several variables as predictors against birth
weight. Specifically, these regression analyses will focus on
demographic information and measurements taken of the child at birth.
The selected variables of interest include:
...
changepoint at 35 weeks for gestational age. The motivation for simply
using one factor is to assess if gestational age alone can account for
trends seen in birth weight. Throughout childhood it is noted that
children follow standardized growth charts and I was curious if this can
be applied to gestational age and birth weight. As a note, the
changepoint was determined visually upon inspecting the data in a
scatter plot and observing that a single linear model would not be
appropriate. The resulting plot with the model as well as a plot of
residuals against the fitted values are both shown below.
...
predictors. However, the improved correlation of data based on the
models does not suggest conclusions can be made on causation or any
other interactions.
...
and tmax
*β*̂<sub>1</sub>
) also appears to have a relatively normal distribution about 7.5 but
has a much sharper peak rather than being smooth.
